...
const loadStats = async () => {
  ...
  try {
    ...
    message.error('加载统计信息失败: ' + (error.response?.data?.error || error.message));
  } finally {
    loading.value = false;
  }
};
...
onMounted(() => {
  loadStats();
  // 监听账号或 API 密钥保存事件，刷新统计
  window.addEventListener('account-saved', loadStats);
  window.addEventListener('api-key-saved', loadStats);
});
